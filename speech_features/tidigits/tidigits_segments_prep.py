...
def main():
    ...
    with open(fa_fn) as f:
        ...
        for line in f:
            ...
            start_rounded = int(np.floor(digit_start*100))/100.0
            end_rounded = int(np.floor((digit_start + digit_duration)*100))/100.0

            segment_key = "{}_{}_{}_{:06d}-{:06d}".format(
                digit_key,
                utt_key,
                chr(utt_digit_keys[utt_key][digit_key] + ord("a")),
                int(start_rounded * 100),
                int(end_rounded * 100))
            segments[segment_key] = (utt_key, start_rounded, end_rounded)
